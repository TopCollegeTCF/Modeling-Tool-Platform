...
export class SecondaryToolbar {
    constructor(editor) {
        ...
        this.registerButton('stencil', {
            title: 'Stencil Tool (🎨)',
            icon: '🎨',
            defaultVisible: true,
            isSystem: false,
            onClick: () => this.toggleStencil(),
        });
    ...
    toggleStencil() {
        if (this.editor.stencilUI) {
            this.editor.stencilUI.toggle();
            // Если панель открыта - активируем инструмент
            if (this.editor.stencilUI.isVisible) {
                this.editor.toolManager.switchTool('stencil');
            } else {
                // Если панель закрыта - возвращаемся к select
