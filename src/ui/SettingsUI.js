...
import { LIGHT_TYPES } from '../services/LightingManager.js';
...
export class SettingsUI {
    ...
    render() {
        ...
        // Lighting Section
        const lightingManager = this.editor.sceneManager?.lightingManager;
        if (lightingManager) {
            const currentType = lightingManager.getType();
            const types = lightingManager.getTypes();
            ...
            types.forEach(type => {
                if (type !== LIGHT_TYPES.CUSTOM) {
                    const isActive = type === currentType;
                    lightsHtml += `
                         <button onclick="window.editor.sceneManager.setLightingType('${type}'); window.editor.settingsUI.render();"
                                 style="padding: 4px 12px; border: 1px solid ${isActive ? '#4a9eff' : 'rgba(255,255,255,0.08)'};
                                        ...
                                        transition: all 0.2s;">
                             ${lightingManager.getTypeLabel(type)}
                         </button>
                     `;
                }
            });
    ...
    applyTheme(theme) {
        ...
        document.querySelectorAll('input, .prop-input, select').forEach(el => {
