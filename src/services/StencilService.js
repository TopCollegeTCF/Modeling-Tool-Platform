 ...
 */
 ...
 export class StencilService {
     ...
     /**
      * Основной метод: обрезает нижнюю часть от уровня трафарета
      */
     applyStencilCut(entity, points) {
         ...
         for (let i = 0; i < vertexCount; i++) {
             ...
             // Нормализованная высота (0 = низ, 1 = верх)
             const normalizedY = (y - minY) / heightRange;
             
             // Вычисляем угол и текущий радиус
             const angle = Math.atan2(z, x);
             const currentRadius = Math.sqrt(x * x + z * z);
             
             // Находим границу трафарета в этом направлении
             const boundary = this.getBoundaryAtAngle(normalizedPoints, angle);
             const targetRadius = boundary * stencilRadius;
             
             let newX, newY, newZ;
             
             // Обрезаем нижнюю половину (от 0 до 50% высоты)
             if (normalizedY < 0.5) {
                 // Прогресс от дна до середины (0 -> 1)
                 const progress = normalizedY / 0.5;
                 
                 // Используем smoothstep для плавности
                 const t = progress * progress * (3 - 2 * progress);
                 
                 // Новый радиус: от targetRadius (внизу) до currentRadius (на середине)
                 const newRadius = targetRadius + (currentRadius - targetRadius) * t;
                 const scale = currentRadius > 0.001 ? newRadius / currentRadius : 1;
                 
                 // Применяем масштабирование к X и Z
                 newX = x * scale;
                 newZ = z * scale;
                 
                 // Y: плавно поднимаемся от уровня трафарета к оригинальной высоте
                 const yProgress = progress;
                 const yEase = yProgress * yProgress;
                 newY = stencilY + (y - minY) * yEase;
                 
                 // Гарантируем, что нижние точки точно на уровне трафарета
                 if (progress < 0.05) {
                     newY = stencilY;
                 }
                 
             } else {
                 // Верхняя половина - остается без изменений, смещаем по Y
